...
negsamplefn <- function(screen_strategy, MISCLASS, PSA) {
  ...
  if (MISCLASS) {
    load("Risksamplewithmisclass/negsample.Rdata")
  } else {
  ...
  if (MISCLASS) {
    ...
    if (screen_strategy == 1 | screen_strategy == 9) {
      negsample$risk_group <- 1 +
        findInterval(negsample$tenyrrisk_est, risk_cutoffs_procas)
    } else if (screen_strategy == 2) {
      negsample$risk_group <- 1 +
        findInterval(negsample$tenyrrisk_est, risk_cutoffs_tert)
      negsample$risk_group <- negsample$risk_group + 2
    } else if (screen_strategy == 7 | screen_strategy == 8) {
      negsample$risk_group <- ifelse(
        negsample$tenyrrisk_est < low_risk_cut,
        1,
        2
      )
    }
  ...
  if (MISCLASS == FALSE) {
    if (screen_strategy == 1 | screen_strategy == 9) {
      negsample$risk_group <- 1 +
        findInterval(negsample$tenyrrisk, risk_cutoffs_procas)
    } else if (screen_strategy == 2) {
      negsample$risk_group <- 1 +
        findInterval(negsample$tenyrrisk, risk_cutoffs_tert)
      negsample$risk_group <- negsample$risk_group + 2
    } else if (screen_strategy == 7 | screen_strategy == 8) {
  ...
  if (
    ...
    if (PSA == 1) {
      savePSA <- as.data.frame(negsample[c(
        "risk_group",
        "interval_change",
    ...
    negsample <- data.frame(
      "risk_group" = negsample$risk_group,
    ...
    if (PSA == 1) {
      ...
      negsample$cost_follow_up <- (1 + savePSA$PSA_cost_follow_up) *
        cost_follow_up_base
    ...
    for (ii in 1:length(subsamples)) {
      ...
      if (
        negsample$risk_group[1] == 3 |
          negsample$risk_group[1] == 2 |
          negsample$risk_group[1] == 0
      ) {
        screen_times <- low_risk_screentimes
      }
      if (
        negsample$risk_group[1] == 1 &
          (screen_strategy == 9 | screen_strategy == 7)
      ) {
      ...
      for (i in 1:(length(screen_times) - 1)) {
        negsample[, 12 + i] <- ifelse(
          rowSums(negsample[12:(11 + i)]) >= 1,
      ...
      qalylookup <- data.frame(
        "age" = seq(from = screen_startage, to = 100, by = 1),
      ...
      for (i in 1:length(qalylookup$qalyweight)) {
        qalylookup$qalyweight[i] <- utility_ages[
          match((ceiling((screen_startage - 1) + i)), utility_ages[, 1]),
          2
        ] *
          (1 / (1 + discount_health)^(i))
      }
      ...
      negsample$QALY <- (qalylookup[
        match(floor(negsample$life_expectancy), qalylookup[, 1]),
        3
      ]) +
        ((negsample$life_expectancy - floor(negsample$life_expectancy)) *
          ((qalylookup[
            match(ceiling(negsample$life_expectancy), qalylookup[, 1]),
            2
          ]) -
            (qalylookup[
              match(floor(negsample$life_expectancy), qalylookup[, 1]),
      ...
      results <- data.frame(
        ...
        rep(0, length = length(negsample$risk_group)),
        negsample$life_expectancy -
          rep(start_age, length = length(negsample$risk_group)),
        rep(0, length = length(negsample$risk_group)),
        rep(0, length = length(negsample$risk_group)),
        negsample$life_expectancy,
        rep(0, length = length(negsample$risk_group))
      ...
      save(
        results,
        file = paste(
          ifelse(PSA == 0, det_output_path, psa_output_path),
          screen_strategy,
          "_",
          ii,
          "_",
