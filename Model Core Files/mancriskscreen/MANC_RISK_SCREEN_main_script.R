    ...
    ) %dopar%
      {
        ...
        if (PREVENTATIVE_DRUG & risk_data$risk_group != 0) {
          ...
          if (cost_in_full_courses) {
            ...
            prop_drug_admin <- time_taking_drug /
        ...
        while (
          ...
          if (screen_detected_ca == 1 || interval_ca == 1) {
            ...
            if (PSA == 0) {
              if (stage_cat < 5) {
                costs <- costs +
                  (as.numeric(fnLookupBase(
                    iStage,
                    iAge,
                    min(c(round(Ca_mort_age - age), 9))
                  )) *
                    current_discount)
              }
            } else {
        ...
        if (PREVENTATIVE_DRUG & risk_data$risk_group != 0) {
          # Don't model impact of drug for strategies without risk stratification
          # Decide if individual takes drugs and add cost if so
          if (
            dqrunif(1, 0, 1) <
              uptake[risk_data$risk_group, risk_data$starting_menses_status]
          ) {
            time_taking_drug <- min(
              rexp(
                1,
                rate = persistence[
                  risk_data$risk_group,
