# Block of code for setting things up to do profiling on individual life history
# sims.

rm(list = ls())

# Reading in code using source() function always gives same random seed, this
# should mean that what we get is truly random:
# seed_arg <- Sys.time() %>% as.numeric()
# set.seed(seed_arg)
...
i <- sample(nrow(splitsample), size = 1)
...
if (PSA == 1) {
  ...
  risk_data$growth_rate <- risk_data$cancer *
    qlnorm(dqrunif(1, 0, 1), meanlog = log_norm_mean, sdlog = sqrt(log_norm_sd))
