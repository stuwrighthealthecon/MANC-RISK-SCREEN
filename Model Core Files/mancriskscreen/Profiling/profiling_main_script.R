...
profvis({
  ...
  if (risk_data$cancer == 1) {
    ...
    if (ca_incidence_i[2] == 1) {
      ...
      CD_age <- ca_incidence_i[1] +
        ((log((Vm / Vc)^0.25 - 1) -
          log((Vm / ((4 / 3) * pi * (ca_incidence_i[4] / 2)^3))^0.25 - 1)) /
          (0.25 * grow_rate_i)) -
        ((log((Vm / Vc)^0.25 - 1) -
          log((Vm / ((4 / 3) * pi * (ca_incidence_i[3] / 2)^3))^0.25 - 1)) /
          (0.25 * grow_rate_i))
    }
      ...
      if (
        ...
        if (Event_place == 1 && ca_case == 1) {
          ...
          if (t > 0) {
            ...
            if (screen_result[1] == 1 && screen_count == length(screen_times)) {
              sdlast_cancer <- 1
    ...
    if (screen_detected_ca == 1 || interval_ca == 1) {
      ...
      if (PSA == 0) {
        if (stage_cat < 5) {
          costs <- costs +
            (as.numeric(
              fnLookupBase(iStage, iAge, min(c(round(Mort_age - age), 50))) *
                current_discount
            ))
        }
      } else {
  ...
  for (y in 1:length(QALY_vect)) {
    QALY_vect[y] <- (utility_ages[
      match((ceiling(((screen_startage - 1) + y) / 5) * 5), utility_ages[, 1]),
      2
    ]) *
...
#### Should find from previous section that around 50% of time is spent in the
####  while ((age < Mort_age) && (interval_ca == 0) && (screen_detected_ca == 0))
#### loop. Try profiling interior of this loop:

source(file = "setup_profiling.R")
...
while ((age < Mort_age) && (interval_ca == 0) && (screen_detected_ca == 0)) {
  ...
  if (Event_place == 1) {
    ...
    if (
      ...
      costs <- costs + (cost_screen * current_discount)
      if (
        screen_count == 1 & screen_strategy < 3 & risk_data$risk_predicted == 1
      ) {
        costs <- costs + (cost_strat * current_discount)
      }
      if (
      ...
      if (
        screen_count == 1 & screen_strategy == 9 & risk_data$risk_predicted == 1
      ) {
        costs <- costs + (cost_strat * current_discount)
      }
      if (screen_count == length(screen_times)) {
  ...
  cat("Evaluation up to age", age, "took", elapsed, "seconds.\n")
}
cat("Risk status is", risk_data$cancer, "\n")
