...
output_df$qaly <- alldata %>%
  ...
  summarize(Mean = mean(QALY, na.rm = TRUE)) %>%
  dplyr::select(Mean)
...
output_df$screens <- alldata %>%
  group_by(alternative) %>%
  summarize(Mean = mean(Screens, na.rm = TRUE)) %>%
  dplyr::select(Mean)
output_df$screens <- unlist(output_df$screens)

output_df$`life years` <- alldata %>%
  group_by(alternative) %>%
  summarize(Mean = mean(`Life Years`, na.rm = TRUE)) %>%
  dplyr::select(Mean)
output_df$`life years` <- unlist(output_df$`life years`)

#Calculate Incremental Results
output_df[, "incQALYS"] <- output_df$qaly -
  rep(output_df$qaly[1], length = nrow(output_df))
...
fnIncCU <- function(
  ...
  for (i in 2:nn) {
    ...
    IncCU$extdom[i] <- if (i == nn) {
      FALSE
    } else {
      any(
        IncCU$QALY[(i + 1):nn] > IncCU$QALY[i] &
          (IncCU$Cost[(i + 1):nn] - IncCU$Cost[comp]) /
            (IncCU$QALY[(i + 1):nn] - IncCU$QALY[comp]) <
            IncCU$ICER[i]
      )
    }
    IncCU$strICER[i] <- ifelse(
      IncCU$dom[i],
      "dominated",
      ifelse(
        IncCU$extdom[i],
        "extendedly dominated",
        format(round(IncCU$ICER[i], icerDP), nsmall = icerDP, big.mark = ",")
      )
    )
    IncCU$strIncCost[i] <- format(
      round(IncCU$IncCost[i], costDP),
      nsmall = costDP,
      big.mark = ","
    )
    IncCU$strIncQALY[i] <- format(
      round(IncCU$IncQALY[i], qalyDP),
      nsmall = qalyDP,
      big.mark = ","
    )
  }
  ...
  if (blnCUP) {
    ...
    tblXT <- tibble(
      x = ax.x$dMin + (1:nt.x - 1) * ax.x$dMajor,
      y1 = or.y - tl.y / 2,
      y2 = or.y + tl.y / 2
    )
    tblYT <- tibble(
      y = ax.y$dMin + (1:nt.y - 1) * ax.y$dMajor,
      x1 = or.x - tl.x / 2,
      x2 = or.x + tl.x / 2
    )
    ...
    plt <- IncCU %>%
      ...
      geom_hline(yintercept = or.y, linewidth = 1.25) +
      geom_segment(
        data = tblXT,
      ...
      geom_text(aes(label = ID), colour = "white", size = 12 * 1 / 72 * 25.4) +
      scale_x_continuous(
  ...
  prettyIncCU <- IncCU %>%
    ...
    cols_label_with(
      starts_with("NHB"),
      ~ paste0("£", as.numeric(str_sub(.x, 4)) / 1000, "K/QALY")
    ) %>%
    cols_label_with(
      starts_with("rankNHB"),
      ~ paste0("£", as.numeric(str_sub(.x, 8)) / 1000, "K/QALY")
    ) %>%
    tab_spanner(
      label = "Incremental",
      columns = c(strIncCost, strIncQALY, strICER)
    ) %>%
    tab_spanner(label = "Net health benefit", columns = starts_with("NHB")) %>%
    tab_spanner(label = "Rank", columns = starts_with("rankNHB")) %>%
    cols_align(align = "center", columns = -c(StratName)) %>%
    opt_horizontal_padding(scale = 2) %>%
    opt_table_font(font = "Arial")

  return(list(IncCU = IncCU, prettyIncCU = prettyIncCU, CUplane = plt))
}
...
IncCU <- fnIncCU(
  Names = rownames(output_df),
  Costs = output_df$cost,
  QALYs = output_df$qaly
)
IncCU$prettyIncCU
