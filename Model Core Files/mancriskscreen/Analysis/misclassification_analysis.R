...
# Add amount of percentage error in risk prediction
risk_jitter <- 2
...
for (ii in 1:chunks) {
  ...
  if (PSA == 0) {
    ...
    save(
      results,
      file = paste(
        "DeterministicResultsWithMisclass/Determ_",
        screen_strategy,
        "_",
        ii,
        ".Rdata",
...
if (PSA == 0) {
  ...
  write.csv(
    merged_result,
    file = paste("Detresults_MC_strat_", screen_strategy, ".csv")
  )
} else {
  for (i in 1:chunks) {
    #Record average outputs for each chunk and save in an excel file
  ...
  write.csv(
    merged_result,
    file = paste("PSAresults_MC_strat_", screen_strategy, ".csv")
  )
}
