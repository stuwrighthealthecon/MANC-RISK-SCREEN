...
base_data <- (PSA_all) %>%
  select(one_of(c("id","alternative","QALY","cost"))) %>%
...
base_data <- base_data %>%
  mutate(delta_QALY_no_screening = QALY_no_screening - QALY_no_screening) %>%
  mutate(delta_QALY_procas = QALY_procas - QALY_no_screening) %>%
  mutate(delta_QALY_tertiles = QALY_tertiels = QALY_no_screening) %>%
  mutate(delta_QALY_3yr = QALY_3yr - QALY_no_screening) %>%
  mutate(delta_QALY_2yr = QALY_2yr - QALY_no_screening) %>%
  mutate(delta_QALY_5yr = QALY_5yr - QALY_no_screening) %>%
  mutate(delta_QALY_10yr = QALY_10yr - QALY_no_screening) %>%
  mutate(delta_QALY_lowrisk5yr = QALY_lowrisk5yr - QALY_no_screening) %>%
  mutate(delta_QALY_lowrisk6yr = QALY_lowrisk6yr - QALY_no_screening) %>%
  mutate(delta_QALY_fullstrat = QALY_fullstrat - QALY_no_screening) %>%
  
  mutate(delta_cost_no_screening = cost_no_screening - cost_no_screening) %>%
  mutate(delta_cost_procas = cost_procas - cost_no_screening) %>%
  mutate(delta_cost_tertiles = cost_tertiels = cost_no_screening) %>%
  mutate(delta_cost_3yr = cost_3yr - cost_no_screening) %>%
  mutate(delta_cost_2yr = cost_2yr - cost_no_screening) %>%
  mutate(delta_cost_5yr = cost_5yr - cost_no_screening) %>%
  mutate(delta_cost_10yr = cost_10yr - cost_no_screening) %>%
  mutate(delta_cost_lowrisk5yr = cost_lowrisk5yr - cost_no_screening) %>%
  mutate(delta_cost_lowrisk6yr = cost_lowrisk6yr - cost_no_screening) %>%
  mutate(delta_cost_fullstrat = cost_fullstrat - cost_no_screening)
